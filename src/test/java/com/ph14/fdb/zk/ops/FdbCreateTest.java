...
public class FdbCreateTest extends FdbBaseTest {
  ...
  public void itCreatesADirectory() {
    Result<CreateResponse, KeeperException> result = fdbCreate.execute(REQUEST, transaction, new CreateRequest(BASE_PATH,  new byte[0], Collections.emptyList(), 0));
  ...
  public void itDoesNotCreateTheSameDirectoryTwice() {
    ...
    result = fdbCreate.execute(REQUEST, transaction, new CreateRequest(BASE_PATH, new byte[0], Collections.emptyList(), 0));
  ...
  public void itDoesNotCreateDirectoryWithoutParent() {
    Result<CreateResponse, KeeperException> result = fdbCreate.execute(REQUEST, transaction, new CreateRequest(SUBPATH, new byte[0], Collections.emptyList(), 0));
  ...
  public void itProgressivelyCreatesNodes() {
    ...
    result = fdbCreate.execute(REQUEST, transaction, new CreateRequest(SUBPATH, new byte[0], Collections.emptyList(), 0));
    assertThat(result.isOk()).isTrue();
