...
public class FdbCreateTest extends FdbBaseTest {
  ...
  public void itCreatesADirectory() {
    Result<CreateResponse, KeeperException> result = new FdbCreate(transaction, new CreateRequest(BASE_PATH,  new byte[0], Collections.emptyList(), 0)).execute();
  ...
  public void itDoesNotCreateTheSameDirectoryTwice() {
    ...
    result = new FdbCreate(transaction, new CreateRequest(BASE_PATH, new byte[0], Collections.emptyList(), 0)).execute();
  ...
  public void itDoesNotCreateDirectoryWithoutParent() {
    Result<CreateResponse, KeeperException> result = new FdbCreate(transaction, new CreateRequest(SUBPATH, new byte[0], Collections.emptyList(), 0)).execute();
  ...
  public void itProgressivelyCreatesNodes() {
    ...
    result = new FdbCreate(transaction, new CreateRequest(SUBPATH, new byte[0], Collections.emptyList(), 0)).execute();
    assertThat(result.isOk()).isTrue();
